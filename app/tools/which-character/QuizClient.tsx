  ...
  const runMatchingAlgorithm = (finalAnswers: Record<string, number>) => {
    ...
    setTimeout(() => {
      ...
      characters.forEach((char) => {
        ...
        // Apply dynamic character matched roles based on stats_json.role and affiliation attributes
        const role = String(char.stats_json?.role || "").toLowerCase()
        const affiliation = String(char.stats_json?.affiliation || "").toLowerCase()

        if (role.includes("mastermind") || role.includes("planner") || role.includes("leader")) {
          charProfile.loyalty += 4
          charProfile.street_smart += 3
          charProfile.tech_savvy += 3
          charProfile.stealth += 2
        } else if (role.includes("enforcer") || role.includes("muscle") || role.includes("fighter") || role.includes("shooter")) {
          charProfile.aggression += 4
          charProfile.chaos += 4
          charProfile.stealth += 3
          charProfile.street_smart += 2
        } else if (role.includes("hacker") || role.includes("tech") || role.includes("operator")) {
          charProfile.tech_savvy += 5
          charProfile.stealth += 3
        } else if (role.includes("driver") || role.includes("racer") || role.includes("wheelman")) {
          charProfile.street_smart += 4
          charProfile.chaos += 3
        }

        if (affiliation.includes("crew") || affiliation.includes("partner") || affiliation.includes("duo")) {
          charProfile.loyalty += 3
        }
