...
import { Search, FileText, Swords, Loader2 } from "lucide-react"
...
export default function SearchClient() {
  ...
  useEffect(() => {
    const performSearch = async () => {
      ...
      if (trimmed.length < 2) {
        setResults({ articles: [], characters: [] })
        setLoading(false)
        return
      }
      ...
      setLoading(true)
      try {
        const [articlesRes, charactersRes] = await Promise.all([
          supabase
            .from("articles")
  ...
  const hasAnyResults =
    results.articles.length > 0 || results.characters.length > 0
    ...
    <div className="space-y-8">
      ...
      <div className="relative max-w-2xl mx-auto">
        ...
        <input
          ...
          placeholder="Type keywords to search articles or cast lore..."
